...
function appelAjaxGenereFic()
{
	...
	xhr.onreadystatechange = function()
	{
		...
		if(xhr.readyState == 4 && xhr.status == 200) {
			...
			document.getElementById("contenu").innerHTML= laReponse;
		}
	}
	// Ici on ouvre la page ajax ou l'on souhaite traiter l'information
	xhr.open("POST","ajax/genereFichier.php",true);
	// ne pas oublier ça pour l'entete
	xhr.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
	// ne pas oublier de poster les arguments
	// ici, la valeur A qui correspond à champsA
	valA = document.getElementById("contenu").innerHTML;
	xhr.send("personne="+valA ); // on indique le nom du Post (valeurA) et sa valeur valA
}
