...
<html>
        ...
        <script language="JavaScript" type="text/JavaScript">
            ...
            */
            ...
            function premierAppelAjax()
            {
                ...
                {
                    tabIndivdu[i].setNbiteration(0);
                    eval('document.monForm.nbFois'+i+'.value='+tabIndivdu[i].nbiteration);
                    eval('document.monForm.nbSelec'+i+'.value='+tabIndivdu[i].nombreSelection);
                    
                    if (eval("document.monForm.cle"+i+".checked"))
                    {
                        tabIndivdu[i].setCocheTF(true);
                        nbCoche = nbCoche + 1;
                    }
                    else
                    {
                        tabIndivdu[i].setCocheTF(false);
                    }
                }
                ...
                else
                {
                    ...
                    document.getElementById("nbTourRestant").innerHTML = "Veuillez cocher au moins une case.";
                }
            }
            ...
            function appelAjax()
            {
                ...
                xhr.onreadystatechange = function()
                {
                    ...
                    if(xhr.readyState == 4 && xhr.status == 200) {
                        ...
                        if (fin == true) 
                        {
                            ...
                            eval('personne'+persmax+'.setNombreSelection(personne'+persmax+'.nombreSelection+1);');

                            // supprime la couleur rouge du dernier aléatoire
                            eval('document.getElementById("nom'+nbAleatoire+'").style.color   = "black";');
                            eval('document.getElementById("count'+nbAleatoire+'").style.color = "black";');
                            
                            // met en rouge le candidat sélectionné
                            ...
                            if (nbCoche == 1)
                            {
                                for (i = 0; i <= maxnbpers; i++)
                                {
                                    eval("document.monForm.cle"+i+".checked=true;");
                                }
                            }
                            else
                            {
                ...
                envoi = "maxnbpers="+maxnbpers;
                for (i = 0; i <= maxnbpers; i++) 
                {
                    eval('document.monForm.nbFois'+i+'.value='+tabIndivdu[i].nbiteration);

                    if (tabIndivdu[i].cocheTF == true)
                    {
    ...
    <body>
        <?php
            ...
            foreach ($indiv as $key => $individu) 
            {
                echo '<input type="checkBox" name="cle'.$key.'" checked>'.$individu->infosIndividu().'<input type="hidden" name="nbFois'.$key.'" size=1><input type="hidden" name="nbSelec'.$key.'" size=1><br/>';
            }
        ?>
